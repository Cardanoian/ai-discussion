...
export const registerRoomHandlers = (io: Server, socket: Socket) => {
    ...
    socket.on('create_room', ({ userId, subjectId }: { userId: string, subjectId: string }, callback) => {
        const roomId = `room_${new Date().getTime()}`;
        
        supabase.from('subjects').select('*').eq('uuid', subjectId).single().then(({ data, error }) => {
            if (error || !data) {
            ...
            io.emit('rooms_update', rooms.filter(r => !r.isFull && !r.battleStarted));
        });
    });

    socket.on('join_room', ({ roomId, userId }: { roomId: string, userId: string }, callback) => {
    ...
    socket.on('player_ready', ({ roomId, userId }: { roomId: string, userId: string }) => {
        const room = rooms.find(r => r.roomId === roomId);
        if (room) {
